...
export class StorageService {
  ...
  async initialize(): Promise<void> {
    ...
    await this.ensureDirectory(`${this.configDir}/attachments`);
  ...
  async cleanupOldAttachments(): Promise<void> {
    const attachmentsDir = `${this.configDir}/attachments`;
    
    try {
      const files = await this.plugin.app.vault.adapter.list(attachmentsDir);
      ...
      for (const filePath of files.files) {
        try {
          ...
          console.warn(`Failed to check/delete attachment ${filePath}:`, error);
        }
      }

      if (deletedCount > 0) {
        console.log(`Cleaned up ${deletedCount} old attachments`);
      }
    } catch (error) {
